...
function getAABB2FromRect(out, position, angle, size) {
    const points = getAABB2_points, hw = size[0] * 0.5, hh = size[1] * 0.5;
    AABB2_1.AABB2.identity(out);
    gl_matrix_1.vec2.set(points[0], position[0] - hw, position[1] - hh);
    gl_matrix_1.vec2.set(points[1], position[0] - hw, position[1] + hh);
    gl_matrix_1.vec2.set(points[2], position[0] + hw, position[1] + hh);
    gl_matrix_1.vec2.set(points[3], position[0] + hw, position[1] - hh);
    points.forEach((point) => {
        gl_matrix_1.vec2.rotate(point, point, position, angle);
        AABB2_1.AABB2.expandPoint(out, out, point);
    });
    return out;
}
exports.getAABB2FromRect = getAABB2FromRect;
