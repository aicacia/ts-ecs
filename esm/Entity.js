...
export class Entity extends ToFromJSONEventEmitter {
    ...
    validateRequirements() {
        ...
        for (const [Component, component] of this.components) {
            const missingRequiredComponents = filterRequirements(component.getRequiredComponents(), (C) => !this.hasComponent(C)), missingRequiredPlugins = filterRequirements(component.getRequiredPlugins(), (P) => !this.getRequiredScene().hasPlugin(P));
        ...
        if (missingComponents.size > 0 || missingPlugins.size > 0) {
            const componentMessage = [...missingComponents.entries()]
                ...
                .map((missingRequirement) => `Entity's ${requirementToString(component)} requires ${requirementToString(missingRequirement)} Component`)
                .join("\n"))
                .join("\n"), pluginMessage = [...missingPlugins.entries()]
                .map(([component, missingRequirements]) => missingRequirements
                .map((missingRequirement) => `Entity's ${requirementToString(component)} requires ${requirementToString(missingRequirement)} Plugin`)
                .join("\n"))
                .join("\n");
    ...
    fromJSON(json) {
        ...
        if (isJSONArray(json.components)) {
            this.addComponents(json.components.map((componentJSON) => Component.newFromJSON(componentJSON)));
        }
        return this;
    }
}
