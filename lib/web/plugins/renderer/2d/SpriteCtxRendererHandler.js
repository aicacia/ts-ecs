        ...
        this.getManager(Sprite_1.SpriteManager).ifSome((spriteManager) => {
            ...
            for (const sprite of spriteManager.getComponents()) {
                ...
                if (sprite.getRenderable() && imageOption.isSome()) {
                    const image = imageOption.unwrap(), transform = TransformComponent_1.TransformComponent.getRequiredTransform(sprite.getRequiredEntity());
                    math_1.getAABB2FromRect(aabb, transform.getPosition2(tmp0), transform.getRotationZ(), sprite.getSize(tmp1));
